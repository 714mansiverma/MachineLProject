...
class DataTransformation:
    ...
    def get_data_transformer_obj(self):
        ...
        try:
            ...
            logging.info('Numerical Columns Transformation Done.')
            logging.info('Categorical Columns Encoding Completed.')

            preprocess=ColumnTransformer(
                [
                    ('numrica_pipeline',num_pipeline,numerical_columns),
    ...
    def initiate_data_tranformation(self,train_path,test_path):
        try:
            ...
            logging.info('process object obtained successfully.')
            input_train_arr=preprocessor.fit_transform(input_train_df)
            input_test_arr=preprocessor.transform(input_test_df)

            train_arr=np.c_[
                input_train_arr,np.array(target_feature_train_df)
            ]
            test_arr=np.c_[
                input_test_arr,np.array(target_feature_test_df)
            ]
